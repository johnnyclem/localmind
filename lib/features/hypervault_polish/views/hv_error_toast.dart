...
/// pattern-matching [HvApiError] into distinct copy (waitlisted, rate
/// limited, service unavailable, unauthorized) and falling back to the raw
/// error/exception message otherwise. Every HyperVault screen should call
/// this instead of hand-rolling its own `SnackBar(content: Text(e.error))`,
/// so failure copy stays consistent app-wide (see the individual
/// `hypervault_*` feature screens for the pre-standardization pattern this
/// replaces).
...
_HvErrorCopy _copyFor(Object error) {
  if (error is HvApiError) {
    if (error.isWaitlisted) {
