...
/// domain>` URL resolves to.
///
/// This is intentionally navigation-free: it only classifies a [Uri]. See
/// `lib/features/deep_links/providers/deep_link_providers.dart` for the
/// app_links listener that feeds URIs into this parser, and `lib/app.dart`'s
/// `redirect:` callback for where a parsed link actually turns into
/// navigation. Host validation against the connected deployment's canonical
/// `app_url` (from capabilities) is left to callers — this parser accepts
/// any http(s) host so it keeps working for self-hosted deployments, and the
/// custom `hypervault://` scheme regardless of "host" segment.
library;
...
/// Read off [HvDeepLink]s that carry it via [hvDeepLinkNextParam]; kept out
/// of the sealed hierarchy itself since `next` composes with every other
/// link kind rather than being one.
...
/// ([HyperVaultAuthState]/`app_links`), not this router-facing parser's —
